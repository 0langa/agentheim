...
class MemoryTool(BaseTool):
    ...
    def __init__(self, base_dir: str | Path = ".ai-team/memory") -> None:
        ...
        schema = ToolSchema(
            ...
            parameters={
                "operation": ParamSchema(type="string", description="Operation: read, write", enum=["read", "write"], required=True),
                "key": ParamSchema(type="string", description="Memory key", required=True),
                "scope": ParamSchema(type="string", description="Scope: run, repository, global", enum=["run", "repository", "global"], required=True),
                "value": ParamSchema(type="any", description="Value for write operation", required=False),
                "run_id": ParamSchema(type="string", description="Run ID for run-scoped memory", required=False),
            },
    ...
    def invoke(self, params: dict[str, Any], context: ToolContext) -> ToolResult:
        ...
        if operation == "write":
            ...
            try:
                ...
                return ToolResult(success=True, data={"key": key, "scope": scope, "path": str(path)})
