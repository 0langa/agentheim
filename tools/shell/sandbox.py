...
class SandboxConfig:
    ...
    allowed_commands: tuple[str, ...] = (
        "python", "pytest", "dotnet", "cargo", "go",
        "git", "npm", "node", "pip", "poetry",
    )
    ...
    allowed_env_vars: tuple[str, ...] = (
        ...
        "SYSTEMROOT", "COMPUTERNAME", "PYTHONPATH",
        "PIP_INDEX_URL", "PIP_TRUSTED_HOST", "POETRY_CACHE_DIR",
        "CARGO_HOME", "NPM_CONFIG_CACHE", "GIT_SSL_NO_VERIFY",
    )
...
class ShellSandbox:
    ...
    def execute(
        ...
        *,
        ...
        try:
            ...
            raise SandboxViolation(f"Command not found: {command[0]}")
        except OSError as exc:
            raise SandboxViolation(f"OS error executing command: {exc}")
    ...
    def _validate_arg(self, arg: str) -> None:
        ...
        if ".." in normalized.split("/"):
            raise SandboxViolation(
                f"Argument contains path traversal: {shlex.quote(arg)}"
            )

    def _build_env(
        self, env_override: dict[str, str] | None
    ) -> dict[str, str]:
