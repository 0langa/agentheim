...
class LocalDBTool(BaseTool):
    ...
    DANGEROUS_KEYWORDS = ("insert", "update", "delete", "drop", "create", "alter", "replace", "attach", "detach", "vacuum")
    ...
    def __init__(self, repo_root: str | Path = ".") -> None:
        ...
        schema = ToolSchema(
            ...
            parameters={
                ...
                "db_path": ParamSchema(type="string", description="Relative path to SQLite database file", required=True),
                "sql": ParamSchema(type="string", description="SQL query (required for query operation)", required=False),
                "table_name": ParamSchema(type="string", description="Table name (required for describe operation)", required=False),
                "limit": ParamSchema(type="integer", description="Max rows to return for query", default=1000, required=False),
            },
    ...
    def _resolve_db_path(self, raw_path: str, context: ToolContext) -> Path:
        ...
        try:
            ...
            raise ToolSafetyError(f"Database path escapes workspace: {raw_path}")
    ...
    def _sanitize_sql(self, sql: str) -> tuple[bool, str]:
        ...
        if first_token not in self.READ_ONLY_PREFIXES:
            return False, f"SQL must start with a read-only statement (got '{first_token}'). Allowed: {self.READ_ONLY_PREFIXES}"
    ...
    def _describe(self, db_path: Path, table_name: str) -> ToolResult:
        if not table_name:
            return ToolResult(success=False, error="Parameter 'table_name' is required for describe.")
