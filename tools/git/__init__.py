...
class GitTool(BaseTool):
    ...
    def __init__(self, repo_root: str | Path = ".") -> None:
        ...
        schema = ToolSchema(
            ...
            parameters={
                "operation": ParamSchema(type="string", description="Operation: status, diff, diff_patch, commit, clone, push", enum=["status", "diff", "diff_patch", "commit", "clone", "push"], required=True),
                ...
                "target": ParamSchema(type="string", description="Target directory for clone", required=False),
            },
    ...
    def _run(self, args: list[str]) -> tuple[int, str, str]:
        result = subprocess.run(["git", *args], cwd=self.repo_root, capture_output=True, text=True, timeout=30, check=False)
    ...
    def _status(self) -> ToolResult:
        ...
        return ToolResult(success=rc == 0, data=out, error=err or None, metadata={"risk": RiskLevel.NONE.value})
    ...
    def _commit(self, message: str) -> ToolResult:
        ...
        return ToolResult(success=rc == 0, data=out, error=err or None, metadata={"risk": RiskLevel.LOW.value})
    ...
    def _clone(self, url: str, target: str) -> ToolResult:
        ...
        return ToolResult(success=rc == 0, data=out, error=err or None, metadata={"risk": RiskLevel.MEDIUM.value})

    def _push(self) -> ToolResult:
        # HIGH risk — policy engine should gate this
        rc, out, err = self._run(["push"])
        return ToolResult(success=rc == 0, data=out, error=err or None, metadata={"risk": RiskLevel.HIGH.value})
