...
from agentheim.vendor.aictx.llm.base import ChatRequest, ChatResponse, ModelProvider
...
def _load_oci_config(profile: str, config_file: Path | None) -> dict[str, Any]:
    ...
    try:
        result: dict[str, Any] = oci.config.from_file(
            file_location=str(config_file) if config_file else None,
            profile_name=profile,
        )
        return result
    except oci.config.ConfigFileNotFound as exc:
        raise ConfigError(f"OCI config file not found: {exc}") from exc
    except oci.config.InvalidKey as exc:
        raise ConfigError(f"OCI config key invalid: {exc}") from exc
...
def _build_oci_messages(oci: Any, request: ChatRequest) -> list[Any]:
    ...
    for msg in request.messages:
        ...
        if role == "user":
            messages.append(
                oci.generative_ai_inference.models.UserMessage(role="USER", content=content)
            )
        elif role == "assistant":
            messages.append(
