...
from ai_team.errors import ResumeError
...
def load_final_report(repo_root: str | Path, run_id: str) -> FinalReport:
    run_dir = Path(repo_root) / ".ai-team" / "runs" / run_id
    report_file = run_dir / "final_report.json"
    if not report_file.exists():
        raise ResumeError(f"Final report missing for run '{run_id}'.")
    return FinalReport.model_validate_json(report_file.read_text(encoding="utf-8"))