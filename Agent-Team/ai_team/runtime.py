...
from ai_team.integrations.web_research import WebResearchAdapter
...
def plan_task(task_text: str, repo_path: str | Path, write_ledger: bool = False) -> tuple[RepoScanResult, str, ImplementationPlan, Path | None]:
    ...
    if write_ledger:
        ...
        if result.parsed_output is not None:
            ...
            ledger.append_jsonl("state_transitions.jsonl", {"state": "planning_succeeded"})
        else:
            ledger.append_jsonl("state_transitions.jsonl", {"state": "planning_failed", "error": result.error})
...
def _write_run_command_output(ledger: RunLedger, index: int, command: list[str], stdout: str, stderr: str) -> None:
    ...
    ledger.append_jsonl("tool_calls.jsonl", {"tool": "shell.run", "command": command, "stdout_file": f"command_{index:02d}_stdout.txt", "stderr_file": f"command_{index:02d}_stderr.txt"})
...
def _basic_verify(
    ...
    for command in work_order.expected_commands:
        if not can_auto_run(command):
            ...
            continue
        try:
            run_result = tools.shell.run(command, timeout_seconds=command_timeout)
    ...
    *,
    ...
    try:
        ...
        for task in plan.task_graph.ordered_tasks:
            ...
            task_verifications, task_commands = _basic_verify(task.work_order, tools, ledger, command_timeout, no_tests)
            ...
            while mode == "auto" and verify_report.status == "failed":
                ...
                if repeated_failures[failure_key] >= 2:
                    ...
                    report = FinalReport(
                        ...
                        next_command_suggestions=[f"python -m ai_team report --repo . --run-id {ledger.run_dir.name}"],
                        status="blocked",
                    )
                ...
                if fix_attempt >= max_fix_attempts:
                    ...
                    report = FinalReport(
                        ...
                        remaining_risks=[*verify_report.failed_checks, *verify_report.regressions],
                        run_id=ledger.run_dir.name,
                        next_command_suggestions=[f"python -m ai_team resume --repo . --run-id {ledger.run_dir.name}"],
                        status="blocked",
                    )
                    ledger.write_json("final_report.json", report.model_dump())
                    ledger.write_text("final_report.md", render_final_report_markdown(report))
                ...
                while attempts_remaining > 0 and not applied:
                    patch_attempt_index += 1
                    coder_result = coder.run_work_order(work_order, repo_root)
                    ledger.append_jsonl("patch_attempts.jsonl", {"task_id": task.id, "attempt": patch_attempt_index, "success": coder_result.success, "raw_output": coder_result.raw_output, "fix_loop": True})
                    if not coder_result.success or coder_result.parsed_output is None:
                ...
                task_verifications, task_commands = _basic_verify(work_order, tools, ledger, command_timeout, no_tests)
                verification_records.extend(task_verifications)
                commands_run.extend(task_commands)
        ...
        report = FinalReport(
            ...
            run_id=ledger.run_dir.name,
            next_command_suggestions=[] if verification_records else ["python -m ai_team inspect --repo . --write-ledger"],
            status="done",
        )
        ledger.write_json("final_report.json", report.model_dump())
        ledger.write_text("final_report.md", render_final_report_markdown(report))
