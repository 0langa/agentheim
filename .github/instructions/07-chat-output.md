...
These rules govern what an agent may write to the user-facing chat while working.
...
- what the agent is currently thinking
- what the agent is about to inspect
- what the agent just noticed but has not acted on
...
- the agent needs permission for a destructive, credential-bearing, networked, or policy-sensitive action
...
- the agent has partial results that the user explicitly requested before completion
...
The agent should not ask for more information unless the missing information blocks safe or correct execution.