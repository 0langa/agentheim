...
class BudgetLimits:
    ...
    max_agent_invocations: Optional[int] = None
...
class StepBudgetEnforcer:
    ...
    def record_tokens(self, count: int, step_id: Optional[str] = None) -> None:
        """Record token consumption."""
        self._snapshot.tokens_used += count
        self._check_and_emit("max_tokens", self._snapshot.tokens_used, self.limits.max_tokens, step_id)

    def record_tool_call(self, step_id: Optional[str] = None) -> None:
        """Record a tool call."""
        self._snapshot.tool_calls_used += 1
        self._check_and_emit("max_tool_calls", self._snapshot.tool_calls_used, self.limits.max_tool_calls, step_id)

    def record_agent_invocation(self, step_id: Optional[str] = None) -> None:
    ...
    def record_time(self, elapsed_seconds: float, step_id: Optional[str] = None) -> None:
    ...
    def check_budget(self, operation: str, step_id: Optional[str] = None) -> bool:
        ...
        # Check each limit
        exceeded: Optional[tuple[str, int, int]] = None

        if self.limits.max_tokens is not None and self._snapshot.tokens_used >= self.limits.max_tokens:
            exceeded = ("max_tokens", self.limits.max_tokens, self._snapshot.tokens_used)
        elif self.limits.max_time_seconds is not None and int(self._snapshot.time_elapsed_seconds) >= self.limits.max_time_seconds:
            exceeded = ("max_time_seconds", self.limits.max_time_seconds, int(self._snapshot.time_elapsed_seconds))
        elif self.limits.max_tool_calls is not None and self._snapshot.tool_calls_used >= self.limits.max_tool_calls:
            exceeded = ("max_tool_calls", self.limits.max_tool_calls, self._snapshot.tool_calls_used)
        elif self.limits.max_agent_invocations is not None and self._snapshot.agent_invocations_used >= self.limits.max_agent_invocations:
            exceeded = ("max_agent_invocations", self.limits.max_agent_invocations, self._snapshot.agent_invocations_used)
