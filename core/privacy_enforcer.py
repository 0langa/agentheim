...
class PrivacyEnforcer:
    ...
    def evaluate(
        ...
        if self.mode == PrivacyMode.LOCAL_ONLY:
            if tool_id.startswith("http.") or tool_id in {"git.push", "git.clone"}:
                violations.append(
                    f"Network tool '{tool_id}' blocked in local_only mode."
                )
        ...
        if self.mode in {PrivacyMode.STRICT_PRIVATE, PrivacyMode.ENCRYPTED}:
            ...
            if path and self._is_sensitive(str(path)):
                violations.append(
                    f"Sensitive path '{path}' blocked in {self.mode.value} mode."
                )
